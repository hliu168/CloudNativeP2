...
### Steps (Modified by Student)

There are 4 folders inside the modules subfolder. They are usConnection, usLocation, usPerson, and usFrontend, where the "us" prefix stands for uService or microservice.

Inside each of the 4 folders, you will find a deployment folder, these folder contain kubernetes yaml files to deploy the service and its dependencies (including db and kafka queue).

Need to run kubectl apply -f deployment inside the us* subfolders to deploy to kubernetes

Inside usPerson and usLocation folders, you will find a scrips folder to seed database similar to the original starter project. Whereas usPerson's script should run against postgres-person pod and usLocation's script should run against postgres-location pod.
...
* `http://localhost:30003/` - Reserved for gPRC Location deployment/service
* `http://localhost:30004/` - Connection OpenAPI Documentation
